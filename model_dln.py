...
class MDLN_Result(object):
    ...
    def load_data(self, path):
        ...
        self.samples = MDLN_Samples(self.nSamp, self.nDat, self.nCol, self.nMix)
        self.samples.mu = mu
...
MDLN_State = namedtuple('MDLN_State', 'alpha delta r pi mu Sigma temp')
MDLN_Prior = namedtuple('MDLN_Prior', 'pi mu Sigma')

class MDLN_Chain(pt.PTChain):
    @property
    def curr_alpha(self):
    ...
    def get_state(self):
        state = MDLN_State(
                self.curr_alpha, self.curr_delta, self.curr_r, self.curr_pi,
                self.curr_mu, self.curr_Sigma, self.inv_temper_temp,
                )
        return state
    ...
    def initialize_sampler(self, nSamp):
        self.samples = MDLN_Samples(nSamp, self.nDat, self.nCol, self.nMix)
        self.curr_iter = 0
        ...
        self.samples.delta[0] = self.sample_delta(self.curr_r, self.curr_alpha, self.curr_pi)
        return
    ...
    def log_posterior_state(self, state):
        ...
        lp_mu    = log_density_mvnormal(
                state.mu, self.priors.mu.mu, self.priors.mu.SCho, self.priors.mu.SInv,
                )
        lp_Sigma = invwishart(df = self.priors.Sigma.nu, scale = self.priors.Sigma.psi).logpdf(state.Sigma)
        # lp_pi = ?
        return (llik + lp_alpha + lp_mu + lp_Sigma) * self.inv_temper_temp

    def sample_delta(self, r, alpha, pi):
        Y = (self.data.S.T * r).T
        args = zip(Y, repeat(alpha), repeat(pi))
        res = map(sample_delta_i, args)
        return np.array(list(res), dtype = int).reshape(-1)

    def sample_alpha_new(self, mu, Sigma_cho, n):
        log_alpha = mu.reshape(1,-1) + (np.triu(Sigma_cho[0]) @ normal(size = (self.nCol, n))).T
        return np.exp(log_alpha)

    def sample_alpha_j(self, curr_alpha_j, Yj, Sigma_cho, Sigma_inv, mu):
        if Yj.shape[0] == 0:
            return self.sample_alpha_new(mu, Sigma_cho, 1).reshape(-1)
    ...
    def sample_mu(self, Sigma_inv, alpha):
        la_bar = np.log(alpha).mean(axis = 0)
        _Sigma = cho_solve(
    ...
    def sample_Sigma(self, mu, alpha):
        diff = np.log(alpha) - mu
    ...
    def iter_sample(self):
        ...
        self.samples.delta[self.curr_iter] = self.sample_delta(r, alpha, pi)
        self.samples.r[self.curr_iter] = self.sample_r(alpha, self.curr_delta)
        self.samples.alpha[self.curr_iter] = self.sample_alpha(
                alpha, self.curr_delta, self.curr_r, Sigma_chol, Sigma_inv, mu,
                )
    ...
    def __init__(
        ...
        self.priors = MDLN_Prior(prior_pi, prior_mu_actual, prior_Sigma_actual)
        self.r0 = r0
        self.psi0 = psi0
        self.nu = nu
...
class DPDLN_Result(object):
    ...
    def load_data(self, path):
        ...
        self.samples = DPDLN_Samples(self.nSamp, self.nDat, self.nCol)
        self.samples.mu = mu
        self.samples.Sigma = Sigma.reshape(self.nSamp, self.nCol, self.nCol)
        self.samples.delta = deltas
    ...
    def __init__(self, path):
        ...
        self.priors = DPDLN_Prior(prior_mu, prior_Sigma, prior_eta)
        return

DPDLN_State = namedtuple('DPDLN_State', 'alphas delta eta r mu Sigma temp')
DPDLN_Prior     = namedtuple('DPDLN_Prior', 'mu Sigma eta')

class DPDLN_Chain(pt.PTChain):
    @property
    ...
    def initialize_sampler(self, nSamp):
        self.samples = DPDLN_Samples(nSamp, self.nDat, self.nCol)
        self.curr_iter = 0
        self.samples.mu[0] = self.priors.mu.mu + self.priors.mu.SCho @ normal(size = self.nCol)
        self.samples.Sigma[0] = invwishart.rvs(
                df = self.priors.Sigma.nu, scale = self.priors.Sigma.psi,
    ...
    def __init__(
        ...
        self.priors = DPDLN_Prior(prior_mu_actual, prior_Sigma_actual, prior_eta)
        self.r0     = r0
        self.psi0   = psi0
        self.nu     = nu
        self.set_temperature(temperature)
