...
class MPRGLN_Result(object):
    ...
    def load_data(self, path):
        ...
        self.samples = MPRGLN_Samples(self.nSamp, self.nDat, self.nCol, self.nMix)
        self.samples.mu = mu
...
MPRGLN_State = namedtuple('MPRGLN_State', 'alpha delta r pi mu Sigma temp')

class MPRGLN_Chain(pt.PTChain):
    @property
    def curr_alpha(self):
    ...
    def initialize_sampler(self, nSamp):
        self.samples = MPRGLN_Samples(nSamp, self.nDat, self.nCol, self.nMix)
        self.curr_iter = 0
    ...
    def __init__(
        ...
        self.priors = MPRGLN_Prior(prior_pi, prior_mu_actual, prior_Sigma_actual)
        self.r0 = r0
        self.psi0 = psi0
        self.nu = nu
...
class DPPRGLN_Chain(pt.PTChain):
    ...
    def initialize_sampler(self, nSamp):
        self.samples = DPPRGLN_Samples(nSamp, self.nDat, self.nCol)
        self.curr_iter = 0
        self.samples.mu[0] = self.priors.mu.mu + self.priors.mu.SCho @ normal(size = self.nCol)
        self.samples.Sigma[0] = invwishart.rvs(
                df = self.priors.Sigma.nu, scale = self.priors.Sigma.psi,
    ...
    def __init__(
        ...
        self.priors = DPPRGLN_Prior(prior_mu_actual, prior_Sigma_actual, prior_eta)
        self.r0     = r0
        self.psi0   = psi0
        self.nu     = nu
        self.set_temperature(temperature)
