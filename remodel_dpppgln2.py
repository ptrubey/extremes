...
from remodel_dpppgln import dprodgamma_log_multi_theta
...
class Result(object):
    ...
    def __init__(self, path):
        ...
        self.priors = Prior(prior_mu, prior_Sigma, prior_beta, prior_eta)
        self.data = Data(os.path.join(os.path.split(path)[0], 'empirical.csv'))
        return

State = namedtuple('State', 'alphas betas delta eta r mu Sigma temp')
Prior = namedtuple('Prior', 'mu Sigma beta eta')
...
class Chain(pt.PTChain):
    ...
    def get_state(self):
        state = State(self.curr_alphas, self.curr_betas, self.curr_delta, self.curr_eta,
    ...
    def sample_delta_i(self, curr_cluster_state, cand_cluster_state, 
                            eta, delta_i, p, y, alpha, beta, logConstant, temp):
        """ 
        ...
        with np.errstate(divide = 'ignore', invalid = 'ignore'):
            temp[:] = (
                ...
                + dprodgamma_log_multi_theta(y, alpha, beta, logConstant)
                )
        temp -= temp.max()
    ...
    def sample_zeta_new(self, mu, Sigma_chol, n):
        return np.exp(mu.reshape(1,-1) + (np.triu(Sigma_chol[0]) @ normal(size = (self.nCol, n))).T)
    
    def sample_sigma_new(self, xi, tau, n):
        return np.hstack((np.ones((n, 1)), gamma(xi, scale = 1 / tau, size = (n, self.nCol - 1))))
    ...
    def sample_zeta(self, curr_alpha, delta, r, Sigma_cho, Sigma_inv, mu):
    ...
    def sample_sigma(self, alpha, delta, r):
        Y = (self.data.Yp.T * r).T
        nClust = delta.max() + 1
    ...
    def sample_r(self, alphas, betas, delta):
        ...
        beta = betas[delta]
        As = alpha.sum(axis = 1)
        Bs = (self.data.Yp * beta).sum(axis = 1)
    ...
    def write_to_disk(self, path, nburn, thin = 1):
        ...
        df_deltas = pd.DataFrame(deltas, columns = ['delta_{}'.format(i) for i in range(self.nDat)])
        df_rs     = pd.DataFrame(rs, columns = ['r_{}'.format(i) for i in range(self.nDat)])
    ...
    def __init__(
        ...
        self.priors = Prior(prior_mu_actual, prior_Sigma_actual, prior_beta, prior_eta)
        self.r0     = r0
        self.psi0   = psi0
        self.nu     = nu
        self.p      = p
