from scipy.stats import norm as normal, invwishart, beta, gamma
...
from projgamma import to_angular, GammaPrior
...
class Data_From_Raw(Data_From_Raw_Base, Transformer):
    def jitter_off_hypercube(self, V, jitter = 1e-6):
        Vi = V.copy()
        Vi[Vi > (1 - jitter)] = 1 - jitter
        Vi[Vi < jitter] = jitter
        return Vi

    def __init__(self, *args):
        super().__init__(*args)
        self.Vi = self.jitter_off_hypercube(self.V)
        self.pVi = self.probit(self.Vi)
        return

class SamplesDPMP(object):
    mu = None
    mu0 = None
    Sigma = None
    Sigma0 = None
    ...
    def __init__(self, nSamp, nDat, nCol):
        self.mu = []
        self.mu0 = np.empty((nSamp + 1, nCol))
        self.Sigma = []
        self.Sigma0 = np.empty((nSamp + 1, nCol, nCol))
        self.delta = np.empty((nSamp + 1, nDat), dtype = int)
        self.eta = np.empty(nSamp + 1)
        return

BNPMPPrior = namedtuple('BNPMPPrior', 'mu Sigma mu0 Sigma0 eta')
NormalPrior = namedtuple('NormalPrior', 'mu SInv')
InvWishartPrior = namedtuple('InvWishartPrior', 'nu psi')

class DPMP(Transformer):
    samples = None
    ...
    def write_to_disk(self, path, nburn, thin = 1):
        ...
        mu_cols   = ['iter'] + ['mu_{}'.format(i) for i in range(1, self.nCol + 1)]
        ...
        mu0_cols = ['mu0_{}'.format(i) for i in range(1, self.nCol + 1)]
    ...
    def sample_mu_Sigma_new(self, mu0, Sigma0, size):
        ...
        mu_new = mu0 + normal.rvs(size = (size, mu0.shape[0])) @ sig0_chol
    ...
    def sample_mu_i(self, pVi, mu0, Sigma):
        ...
        return _mu + _Sigma_chol @ normal.rvs(size = Sigma.shape[1])
    ...
    def sample_mu0(self, mus, Sigma0):
        ...
        _Sigma_chol = cholesky(_Sigma)
        return _mu + _Sigma_chol @ normal.rvs(size = Sigma0.shape[1])
    ...
    def iter_sample(self):
        ...
        self.samples.mu.append(mus)
        self.samples.Sigma.append(Sigmas)
    ...
    def initialize_sampler(self, ns):
        self.samples = SamplesDPMP(ns, self.nDat, self.nCol)
    ...
    def __init__(
            self,
            data,
            prior_mu = NormalPrior(np.zeros(8), np.eye(8) * 2),
            prior_mu0 = NormalPrior(np.zeros(8), np.eye(8) * 0.25),
            prior_Sigma = InvWishartPrior(10, np.eye(8) * 1),
            prior_Sigma0 = InvWishartPrior(10, np.eye(8) * 1),
        ...
        self.priors = BNPMPPrior(prior_mu, prior_Sigma, prior_mu0, prior_Sigma0, prior_eta)
        self.pool = mp.Pool(8)
        return

class ResultDPMP(Transformer):
    samples = None
    nSamp = None
    nDat = None
    nCol = None
    ...
    def generate_posterior_predictive_probit(self, n_per_sample, m = 20):
        ...
        for i in range(self.nSamp):
            ...
            ljs = njs
            # new_mus = self.samples.mu0[i] + self.samples.Sigma0[i] @ normal.rvs(size = (m, self.nCol))
            # new_Sigmas =
            prob = ljs / ljs.sum()
            ...
            for j in range(n_per_sample):
                new_pVis.append(mus[j] + Sigmas[j] @ normal.rvs(size = self.nCol))
    ...
    def load_data(self, path):
        ...
        self.samples = SamplesDPMP(self.nSamp, self.nDat, self.nCol)
        self.samples.mu = [
