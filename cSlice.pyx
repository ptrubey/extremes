...
cdef class SliceSample(object):
    cpdef double univariate_sample(self, logd, double starter,
    ...
    cpdef double skip_univariate_sample(self, logd, double starter,
                                          double increment_size, int skips):
        cdef double try_value
        try_value = starter
        for _ in range(skips):
            try_value = self.univariate_sample(logd, try_value, increment_size)
        return try_value
