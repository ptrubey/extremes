...
VPG_Prior = namedtuple('VPG_Prior', 'zeta sigma')

class VPG_Samples(object):
    zeta  = None
    sigma = None
    r     = None
...
class VPG_Chain(object):
    ...
    def sample_r(self, zeta, sigma):
        shape = zeta.sum()
        rate  = (self.data.Yl * sigma).sum(axis = 1)
    ...
    def initialize_sampler(self, ns):
        self.samples = VPG_Samples(ns, self.nDat, self.nCol)
        self.curr_iter = 0
    ...
    def __init__(
        ...
        self.priors = VPG_Prior(prior_zeta, prior_sigma)
        return

class VPG_Result(object):
    def load_data(self, path):
        ...
        self.samples = VPG_Samples(self.nSamp, self.nDat, self.nCol)
...
class MPG_Chain(object):
    ...
    def sample_r(self, zeta, sigma, delta):
        ...
        Bs = (self.data.Yl * sigma[delta]).sum(axis = 1)
    ...
    def initialize_sampler(self, ns):
        self.samples = MPG_Samples(ns, self.nDat, self.nCol, self.nMix)
...
class MPG_Result(object):
    ...
    def load_data(self, path):
        ...
        self.samples       = MPG_Samples(self.nSamp, self.nDat, self.nCol, self.nMix)
...
DPPG_Prior = namedtuple('DPPG_Prior', 'eta alpha beta xi tau')

class DPPG_Samples(object):
    zeta  = None
    sigma = None
    alpha = None
    beta  = None
...
class DPPG_Chain(object):
    ...
    def initialize_sampler(self, ns):
        self.samples = DPPG_Samples(ns, self.nDat, self.nCol)
        self.samples.alpha[0] = 1.
        self.samples.beta[0] = 1.
        self.samples.xi[0] = 1.
        self.samples.tau[0] = 1.
    ...
    def __init__(
        ...
        self.nCol = self.data.nCol
        self.nDat = self.data.nDat
        self.priors = DPPG_Prior(prior_eta, prior_alpha, prior_beta, prior_xi, prior_tau)
        return

class DPPG_Result(object):
    ...
    def load_data(self, path):
        ...
        self.samples       = DPPG_Samples(self.nSamp, self.nDat, self.nCol)
        self.samples.delta = deltas
