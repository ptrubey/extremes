...
class Data_From_Raw(Data_From_Raw_Base, Transformer):
    def cast_to_cube(self, A, eps = 1e-5):
        V = A / (pi / 2.)
        V[V > (1 - eps)] = 1 - eps
        V[V < eps] = eps
        return V
...
class DPMP(Transformer):
    ...
    def __init__(
            self,
            data,
            prior_mu = NormalPrior(np.zeros(7), np.eye(7) * 1.),
            prior_mu0 = NormalPrior(np.zeros(7), np.eye(7) * 0.125),
            prior_Sigma = InvWishartPrior(10, np.eye(7) * 1),
            prior_Sigma0 = InvWishartPrior(10, np.eye(7) * 1),
