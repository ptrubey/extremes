...
class VGD_Chain(object):
    ...
    def sample_r(self, zeta, sigma):
        shape = zeta.sum()
        rate  = (self.data.S * sigma).sum(axis = 1)
    ...
    def initialize_sampler(self, ns):
        self.samples = VGD_Samples(ns, self.nDat, self.nCol)
        self.curr_iter = 0
    ...
    def __init__(
        ...
        self.priors = VGD_Prior(prior_zeta, prior_sigma)
        return

class VGD_Result(object):
    def load_data(self, path):
        ...
        self.samples = VGD_Samples(self.nSamp, self.nDat, self.nCol)
...
class MGD_Chain(object):
    ...
    def initialize_sampler(self, ns):
        self.samples = MGD_Samples(ns, self.nDat, self.nCol, self.nMix)
...
class MGD_Result(object):
    ...
    def load_data(self, path):
        ...
        self.samples       = MGD_Samples(self.nSamp, self.nDat, self.nCol, self.nMix)
...
class DPGD_Chain(object):
    ...
    def initialize_sampler(self, ns):
        self.samples = DPGD_Samples(ns, self.nDat, self.nCol)
        self.samples.alpha[0] = 1.
        self.samples.beta[0] = 1.
        self.samples.xi[0] = 1.
        self.samples.tau[0] = 1.
    ...
    def __init__(
        ...
        self.nCol = self.data.nCol
        self.nDat = self.data.nDat
        self.priors = DPGD_Prior(prior_eta, prior_alpha, prior_beta, prior_xi, prior_tau)
        return

class DPGD_Result(object):
    ...
    def load_data(self, path):
        ...
        self.samples = DPGD_Samples(self.nSamp, self.nDat, self.nCol)
        self.samples.delta = deltas
